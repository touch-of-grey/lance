...
use lance::dataset::mem_wal::scanner::{
    FtsScoringMode, LsmDataSourceCollector, LsmFtsSearchPlanner, ShardSnapshot,
};
...
async fn run_search(args: &Args) -> Result<serde_json::Value> {
    ...
    let local = run_mode(&planner, FtsScoringMode::Local, &queries, args.k).await?;
    println!(
        "running LocalWithGlobalRescore mode (factor={}) ...",
        args.rescore_factor
    );
    ...
    let rescore_stats = compute_stats(rescore.latencies_us.clone());
    let jaccard = mean_jaccard(&local.top_ids, &rescore.top_ids);
    ...
    println!(
        "rescore: p50={}us p95={}us p99={}us mean={:.0}us qps={:.0}",
        rescore_stats.p50_us,
        rescore_stats.p95_us,
        rescore_stats.p99_us,
        rescore_stats.mean_us,
        rescore_stats.qps
    );
    println!("top-{} jaccard local-vs-rescore = {:.4}", args.k, jaccard);
    ...
    Ok(json!({
        ...
        "rescore_factor": args.rescore_factor,
        "queries": queries.len(),
        "jaccard_local_vs_rescore": jaccard,
        "local": {
        ...
        "rescore": {
            "p50_us": rescore_stats.p50_us,
            "p95_us": rescore_stats.p95_us,
            "p99_us": rescore_stats.p99_us,
            "mean_us": rescore_stats.mean_us as u64,
            "qps": rescore_stats.qps as u64,
        },
    }))
}
