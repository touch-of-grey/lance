...
IVF_PARTITIONS="${IVF_PARTITIONS:-1024}"
RQ_BITS="${RQ_BITS:-8}"
NPROBES="${NPROBES:-16}"
BACKENDS="${BACKENDS:-nvme s3}"
CONFIG_TIMEOUT="${CONFIG_TIMEOUT:-5400}"

LOCAL_DIR="$REPO_ROOT/target/lsm-read-results/${RUN_ID}"
mkdir -p "$LOCAL_DIR" "$CACHE_DIR" "$NVME_PREFIX"

BENCH=mem_wal_lsm_read_bench
...
for backend in $BACKENDS; do
    ...
    for base_rows in $BASE_ROWS_LIST; do
        ...
        # prepare once per (backend, base_rows): base table + btree + IVF_RQ + FTS
        run_phase "prepare_${backend}_${btag}" \
            --phase prepare --uri "$uri" \
            --base-rows "$base_rows" --batch-rows 1000 \
            --vector-dim "$VECTOR_DIM" --ivf-partitions "$IVF_PARTITIONS" --rq-bits "$RQ_BITS" \
        ...
        for k in $K_LIST; do
            ...
            run_phase "$name" \
                ...
                --batch-rows 1000 \
                --queries "$QUERIES" --k "$k" \
                --vector-dim "$VECTOR_DIM" --nprobes "$NPROBES" \
                --cache-dir "$CACHE_DIR" \
                --output "$out"
...
d = sys.argv[1]
# p50 latency (us) per read type across LSM layers.
print(f"{'config':34s} {'layers':>6} {'point_p50':>10} {'vector_p50':>11} {'fts_p50':>9} "
      f"{'point_qps':>10} {'vector_qps':>11} {'fts_qps':>8}")
def g(r, k): return (r.get(k) or {})
for p in sorted(glob.glob(os.path.join(d, "*.json"))):
    if "/prepare" in p or os.path.basename(p).startswith("prepare"): continue
    try:
        r = json.load(open(p))
    except Exception as e:
        print(f"  bad {p}: {e}"); continue
    name = os.path.basename(p)[:-5]
    pl, ve, ft = g(r,"point_lookup"), g(r,"vector"), g(r,"fts")
    print(f"{name:34s} {r.get('flushed_generations','-'):>6} "
          f"{pl.get('p50_us','-'):>10} {ve.get('p50_us','-'):>11} {ft.get('p50_us','-'):>9} "
          f"{pl.get('qps','-'):>10} {ve.get('qps','-'):>11} {ft.get('qps','-'):>8}")
PY
echo ""
